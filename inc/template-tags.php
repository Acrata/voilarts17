...
if ( ! function_exists( 'voilarts17_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function voilarts17_posted_on() {
		...
		$posted_on = sprintf(
			esc_html_x( 'Posted on %s', 'post date', 'voilarts17' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			esc_html_x( 'by %s', 'post author', 'voilarts17' ),
...
if ( ! function_exists( 'voilarts17_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function voilarts17_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'voilarts17' ) );
			if ( $categories_list && voilarts17_categorized_blog() ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'voilarts17' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'voilarts17' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'voilarts17' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'voilarts17' ), esc_html__( '1 Comment', 'voilarts17' ), esc_html__( '% Comments', 'voilarts17' ) );
			echo '</span>';
		}
 ...
 */
function voilarts17_get_svg( $args = array() ) {

	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'voilarts17' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'voilarts17' );
	}
 ...
 */
function voilarts17_get_the_title( $args = array() ) {
 ...
 */
function voilarts17_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'voilarts17' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'voilarts17_content_more_link' );
 ...
 */
function voilarts17_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'voilarts17' ) );
}
add_filter( 'excerpt_more', 'voilarts17_excerpt_more' );
 ...
 * @return string The shortened excerpt.
 */
function voilarts17_get_the_excerpt( $args = array() ) {
 ...
 */
function voilarts17_get_post_image( $size = 'thumbnail' ) {
 ...
 */
function voilarts17_get_post_image_uri( $size = 'thumbnail' ) {
 ...
 */
function voilarts17_get_copyright_text() {

	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'voilarts17_copyright_text' );
 ...
 * @return string
 */
function voilarts17_get_social_share() {
	...
	<div class="social-share">
		<h5 class="social-share-title"><?php esc_html_e( 'Share This', 'voilarts17' ); ?></h5>
		<ul class="social-icons menu menu-horizontal">
			<li class="social-icon">
				<a href="<?php echo esc_url( $twitter_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php echo voilarts17_get_svg( array( 'icon' => 'twitter-square', 'title' => 'Twitter', 'desc' => __( 'Share on Twitter', 'voilarts17' ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Twitter', 'voilarts17' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $facebook_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php echo voilarts17_get_svg( array( 'icon' => 'facebook-square', 'title' => 'Facebook', 'desc' => __( 'Share on Facebook', 'voilarts17' ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Facebook', 'voilarts17' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $linkedin_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=475, height=505' ); return false;">
					<?php echo voilarts17_get_svg( array( 'icon' => 'linkedin-square', 'title' => 'LinkedIn', 'desc' => __( 'Share on LinkedIn', 'voilarts17' ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on LinkedIn', 'voilarts17' ); ?></span>
				</a>
			</li>
		</ul>
	</div><!-- .social-share -->
 ...
 */
function voilarts17_get_mobile_navigation_menu() {
	...
	<nav id="mobile-menu" class="mobile-nav-menu">
		<button class="close-mobile-menu"><span class="screen-reader-text"><?php echo esc_html_e( 'Close menu', 'voilarts17' ); ?></span><?php echo voilarts17_get_svg( array( 'icon' => 'close' ) ); // WPCS: XSS ok. ?></button>
		<?php
			wp_nav_menu( array(
 ...
 */
function voilarts17_get_social_network_links() {
	...
	foreach ( $social_networks as $network ) :
		...
		$network_url = get_theme_mod( 'voilarts17_' . $network . '_link' );
		...
		if ( isset( $network_url ) && ! empty( $network_url ) ) : ?>
			<li class="social-icon <?php echo esc_attr( $network ); ?>">
				<a href="<?php echo esc_url( $network_url ); ?>">
					<?php echo voilarts17_get_svg( array( 'icon' => $network . '-square', 'title' => sprintf( __( 'Link to %s', 'voilarts17' ), ucwords( esc_html( $network ) ) ) ) ); // WPCS: XSS ok. ?>
					<span class="screen-reader-text"><?php echo sprintf( __( 'Link to %s', 'voilarts17' ), ucwords( esc_html( $network ) ) ); // WPCS: XSS ok. ?></span>
				</a>
			</li><!-- .social-icon -->
		<?php endif;
	endforeach; ?>
